...
export default class FilesController {
  static async postUpload(request, response) {
    ...
    if (
      ...
      try {
        let flag = false;
        if (parentId) {
          const folder = await dbClient.filterFiles({
            _id: parentId,
          });
          if (!folder) {
            response
        ...
        if (!flag) {
          const insRes = await dbClient.newFile(
            userId,
            name,
            type,
            isPublic,
  ...
  static async getIndex(request, response) {
    const usrId = request.user._id;
    const _parentId = request.query.parentId
  ...
  static async putPublish(request, response) {
    ...
    const file = await dbClient.filterFiles({
      _id: request.params.id,
    });
    if (!file || String(file.userId) !== String(userId)) {
      response.status(404).json({ error: 'Not found' }).end();
    } else {
      const newFile = await dbClient.updatefiles(
        { _id: file._id },
        { isPublic: true },
      );
  ...
  static async getFile(request, response) {
    ...
    if (!file) {
      ...
      String(file.userId) === String(usrId)
            || file.isPublic
    ) {
      try {
        const content = await UtilController.readFile(
          file.localPath,
        );
